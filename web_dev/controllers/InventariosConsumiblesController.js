module.exports = function(army) {
	...
	var table = 'inventarios_consumibles',
	...
	army.app.get(route + '/existencias', function(req, res) {
		...
		army.sequelize.query("SELECT 	a.idConsumible, b.nombre consumible, \
										SUM(CASE a.tipo WHEN 3 THEN cantidad ELSE 0 END) anterior, \
										SUM(CASE a.tipo WHEN 1 THEN cantidad ELSE 0 END) entradas, \
										SUM(CASE a.tipo WHEN 2 THEN cantidad ELSE 0 END) salidas, \
										SUM(CASE a.tipo WHEN 2 THEN cantidad * -1 ELSE cantidad END) actual \
							FROM inventarios_consumibles a \
							LEFT JOIN consumibles b \
							ON a.idConsumible = b.idConsumible \
							WHERE idCierre = '" + params.idCierre + "' \
							GROUP BY a.idConsumible, b.nombre", { type: army.sequelize.QueryTypes.SELECT })
	...
	army.app.get(route + '/existencias_detalles', function(req, res) {
		...
		army.db.inventarios_consumibles.findAll({
			where: { idConsumible: params.idConsumible },
			include: [
				{
