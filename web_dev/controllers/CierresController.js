module.exports = function(army) {
	...
	army.app.get(route + '/meses', function(req, res) {
		army.db.cierres.findAll({ attributes: [ [army.sequelize.fn('DATE_FORMAT', army.sequelize.col('fechaFin'), '%Y-%m'), 'fecha'] ], group: [army.sequelize.fn('DATE_FORMAT', army.sequelize.col('fechaFin'), '%Y-%m')] })
		.then(function(rows, err) {
