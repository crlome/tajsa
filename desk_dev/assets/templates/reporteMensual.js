define(['/js/base/viewsBase.js'], function (viewsBase) {
	...
	*/
	var ViMain = viewsBase.base.extend({
		el: '#reporteMensual',
		events:{
			'change [data-field="fecha"]': 'change_cbocierre',
		...
		render: function(joptions) {
			...
			function done(data) {
				...
				that.$el.find('[data-field="fecha"]').html(options);

				/*if(joptions.idCierre > 0)
					that.$el.find('[data-field="idCierre"]').val(joptions.idCierre).change();*/
			}
			app.ut.request({url:'/cierres/meses', type:'GET', done:done});
		},
		close: function() {
			...
			if(fecha != -1 && fecha != this.fecha) {
				...
				function done(data) {
					...
					for(var key in pregrupos) {
						...
						var nombre_consumible =  curr[0].consumible.nombre + ' ' + Handlebars.helpers.GetTipoAsfalto(curr[0].tipo_asfalto);

						var resumen = {
							consumible: nombre_consumible,
							tipo: curr[0].consumible.tipo,
							anterior: 0,
							entradas: 0,
							salidas: 0,
							actual: 0,
					...
					var cierre = that.$el.find('[data-field="fecha"] option:selected').text();
					...
					app.ut.request({url:'/producciones/by_fecha_cierre', data:options, done:done_producciones});
				}
				/*var options = {
				...
				app.ut.request({url:'/inventarios_plantas/existencias_mensual', data:{fecha:fecha}, done:done});
				...
				app.ut.request({url:'/inventarios_materiales/existencias_mensual', data:{fecha:fecha}, done:done_materiales});
				...
				app.ut.request({url:'/inventarios_emulsiones/existencias_mensual', data:{fecha:fecha}, done:done_emulsiones});
				...
				app.ut.request({url:'/inventarios_consumibles/existencias_mensual', data:{fecha:fecha}, done:done_consumibles});
			}
		},
		...
		click_pnlconsumibles_table_tbody_tr: function(e) {
			...
			this.popDetalle.render(json, 'consumibles', '/inventarios_consumibles/existencias_detalles');
		},
		click_pnlemulsiones_table_tbody_tr: function(e) {
			...
			this.popDetalle.render(json, 'emulsiones', '/inventarios_emulsiones/existencias_detalles');
		},
		click_pnlmateriales_table_tbody_tr: function(e) {
			...
			this.popDetalle.render(json, 'materiales', '/inventarios_materiales/existencias_detalles');
		},
	});
